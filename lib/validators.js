...
var validateItem = function (validationEntry, key, object) {
    ...
    if (_.isUndefined(objectVal) || _.isNull(objectVal)) {
        ...
        // Arrays do not necessarily need a validator.
        if (isArray){
            errs = errs.concat(
                ArrayValidator.validate(objectVal, validator, opts).errors);
        } else if (validator) {
...
var ArrayValidator = new Validator('array');

ArrayValidator.validate = function (value, validator, options) {

    var errs = [];

    if (!_.isArray(value)) {

        errs.push(new errors.ValidationError(value + 'is not an array'));
    } else  if (validator) {

        // Loop through the values and check them against our validator
        _.each(value, function (val) {
            errs = errs.concat(validator.validate(val).errors);
        });
    }
